...
def load_clip_to_cpu(cfg, zero_shot_model=False):
    ...
    if not zero_shot_model:
        use_flexible_ivlp = cfg.TRAINER.IVLP.USE_FLEXIBLE_IVLP

        design_details = {"trainer": 'FlexibleIVLP' if use_flexible_ivlp else 'IVLP',
                        "vision_depth": cfg.TRAINER.IVLP.FLEX_PROMPT_DEPTH_VISION if use_flexible_ivlp else cfg.TRAINER.IVLP.PROMPT_DEPTH_VISION,
                        "language_depth": cfg.TRAINER.IVLP.FLEX_PROMPT_DEPTH_TEXT if use_flexible_ivlp else cfg.TRAINER.IVLP.PROMPT_DEPTH_TEXT,
                        "vision_ctx": cfg.TRAINER.IVLP.N_CTX_VISION,
                        "language_ctx": cfg.TRAINER.IVLP.N_CTX_TEXT,
                        "prior_fusion_layer_text": cfg.TRAINER.IVLP.PRIOR_FUSION_LAYER_TEXT,
                        "prior_fusion_layer_vision": cfg.TRAINER.IVLP.PRIOR_FUSION_LAYER_VISION,
                        "prior_fusion_lambda_text": cfg.TRAINER.IVLP.PRIOR_FUSION_LAMBDA_TEXT,
                        "prior_fusion_lambda_vision": cfg.TRAINER.IVLP.PRIOR_FUSION_LAMBDA_VISION}
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts, prior_prompt=None):
        x = prompts + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        inputs = [x,0,0,0,prior_prompt]
...
class VLPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        use_flexible_ivlp = cfg.TRAINER.IVLP.USE_FLEXIBLE_IVLP
        text_prompt_depth = (
            cfg.TRAINER.IVLP.FLEX_PROMPT_DEPTH_TEXT
            if use_flexible_ivlp
            else cfg.TRAINER.IVLP.PROMPT_DEPTH_TEXT
        )
        ...
        self.n_cls = n_cls
        self.n_ctx = n_ctx
        self.use_shallow_text_prompt = (not use_flexible_ivlp) and text_prompt_depth > 0
    ...
    def forward(self):
        if self.use_shallow_text_prompt:
            ctx = self.ctx
            if ctx.dim() == 2:
                ctx = ctx.unsqueeze(0).expand(self.n_cls, -1, -1)
        else:
            # Keep the text sequence length unchanged for CLIP's positional
            # embedding and attention mask, but do not use learnable prompt
            # tokens before the late prompt insertion layer.
            ctx = self.ctx.detach()
            if ctx.dim() == 2:
                ctx = ctx.unsqueeze(0).expand(self.n_cls, -1, -1)
...
class ESFG(nn.Module):
    def __init__(self, d_model=512):
        ...
        n_head = d_model // 64
        headdim = d_model // n_head
        self.n_head = n_head
        self.headdim = headdim
        self.d_model = d_model

        self.ln_1 = LayerNorm(d_model)
        self.mlp_q = nn.Sequential(OrderedDict([
            ("c_fc", nn.Linear(d_model, 256)),
            ("gelu", QuickGELU()),
            ("c_proj", nn.Linear(256, d_model))
        ]))
        self.mlp_k = nn.Sequential(OrderedDict([
            ("c_fc", nn.Linear(d_model, 256)),
            ("gelu", QuickGELU()),
            ("c_proj", nn.Linear(256, d_model))
        ]))
    ...
    def attention(self, x: torch.Tensor, y: torch.Tensor):
        ...
        q = x.reshape(x.shape[0], x.shape[1], self.n_head, self.headdim)
        k = y.reshape(y.shape[0], y.shape[1], self.n_head, self.headdim)
        v = y.reshape(y.shape[0], y.shape[1], self.n_head, self.headdim)
        ...
        attn_output = attn_output.reshape(attn_output.shape[0], attn_output.shape[1], self.d_model)
        attn_output = attn_output.transpose(0, 1)

        return attn_output

    def forward(self, x: torch.Tensor, y: torch.Tensor):
        self.mlp_q.to(x.dtype)
        self.mlp_k.to(x.dtype)
        self.gate.to(x.dtype)
        q = self.mlp_q(self.ln_1(x))
        k = self.mlp_k(self.ln_1(y))
        gate = self.gate(self.ln_1(x))
        attn_output = self.attention(q, k)
        x = x + gate * attn_output
        return x
...
class PICM(nn.Module):
    def __init__(self, in_dim, prompt_len, prompt_dim, mid_dim=None):
        super().__init__()
        mid_dim = mid_dim or max(in_dim // 4, 1)
        self.prompt_len = prompt_len
        self.prompt_dim = prompt_dim
        self.down_project = nn.Linear(in_dim, mid_dim)
        self.activation = QuickGELU()
        self.up_project = nn.Linear(mid_dim, prompt_len * prompt_dim)

    def forward(self, x):
        self.down_project.to(x.dtype)
        self.up_project.to(x.dtype)
        prompt = self.up_project(self.activation(self.down_project(x)))
        return prompt.view(x.shape[0], self.prompt_len, self.prompt_dim)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.dtype = clip_model.dtype
        self.ESFG = ESFG()
        self.use_flexible_ivlp = cfg.TRAINER.IVLP.USE_FLEXIBLE_IVLP
        if self.use_flexible_ivlp:
            self.text_prior_adapter = PICM(
                in_dim=clip_model.text_projection.shape[1],
                prompt_len=cfg.TRAINER.IVLP.N_CTX_TEXT,
                prompt_dim=clip_model.ln_final.weight.shape[0]
            )
            self.image_prior_adapter = PICM(
                in_dim=clip_model.visual.output_dim,
                prompt_len=cfg.TRAINER.IVLP.N_CTX_VISION,
                prompt_dim=clip_model.visual.conv1.out_channels
            )
    ...
    def forward(self, image, label=None):
        ...
        fixed_embeddings = self.prompt_learner.fixed_embeddings.to(device=image.device, dtype=self.dtype)
        with torch.no_grad():
            zero_shot_features, _ = self.prompt_learner.ZS_image_encoder(image.type(self.dtype))
            zero_shot_features = zero_shot_features.to(device=image.device, dtype=self.dtype)

        text_prior_prompt = None
        image_prior_prompt = None
        if self.use_flexible_ivlp:
            text_prior_prompt = self.text_prior_adapter(fixed_embeddings)
            image_prior_prompt = self.image_prior_adapter(zero_shot_features)

        prompts = self.prompt_learner()
        basic_text_features = self.text_encoder(prompts, tokenized_prompts, text_prior_prompt)
        h_text_features = basic_text_features
        image_features_global, image_features_local = self.image_encoder(image.type(self.dtype), image_prior_prompt)
        ...
        id_text_features = self.ESFG(h_text_features_id, id_signal).permute(1,0,2)
        ...
        fg_text_features = self.ESFG(h_text_features_fg, fg_signal).permute(1,0,2)
...
class UCEPrompt(TrainerX):
        ...
        for name, param in self.model.named_parameters():
            if name_to_update not in name:
                ...
                if "ZS_image_encoder" in name:
                    param.requires_grad_(False)
                elif name == "prompt_learner.ctx" and (
                    cfg.TRAINER.IVLP.USE_FLEXIBLE_IVLP
                    or cfg.TRAINER.IVLP.PROMPT_DEPTH_TEXT == 0
                ):
                    param.requires_grad_(False)
    ...
    def forward_backward(self, batch):
        ...
        # SAM : B2N Cross-dataset Cross-domain
        # model = self.model
        # optim = self.optim
        # scaler = self.scaler
