...
class ResidualAttentionBlock_FlexibleIVLP(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
                 text_layer=False, i=0, design_details=None):
        super().__init__()
        ...
        self.prompt_start_layer = design_details.get(
            "prompt_start_layer_text" if text_layer else "prompt_start_layer_vision",
            0
        )
        self.first_prompt_layer = max(1, self.prompt_start_layer)
        self.is_first_prompt_layer = i == self.first_prompt_layer
        self.prior_fusion_layer = design_details.get(
            "prior_fusion_layer_text" if text_layer else "prior_fusion_layer_vision",
            -1
        )
        if self.prior_fusion_layer < 0:
            self.prior_fusion_layer = self.first_prompt_layer
        self.is_prior_fusion_layer = i == self.prior_fusion_layer
        self.prior_fusion_lambda = design_details.get(
            "prior_fusion_lambda_text" if text_layer else "prior_fusion_lambda_vision",
            1.0 if text_layer else 0.75
        )

        if i != 0 and i >= self.prompt_start_layer:
            self.add_prompt = add_prompt
            if self.add_prompt:
                if self.text_layer:
    ...
    def forward(self, inputs: torch.Tensor):
        x = inputs[0]
        prior_prompt = inputs[4] if len(inputs) > 4 else None
        if self.add_prompt:
            if not self.text_layer:
                visual_context = self.VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                if self.is_prior_fusion_layer and prior_prompt is not None:
                    prior_context = prior_prompt.permute(1, 0, 2).to(dtype=visual_context.dtype, device=visual_context.device)
                    visual_context = self.prior_fusion_lambda * prior_context + (1 - self.prior_fusion_lambda) * visual_context
                if self.is_first_prompt_layer:
                    prefix = x
                else:
                    prefix = x[0:x.shape[0] - self.n_ctx_visual, :, :]
                x = torch.cat([prefix, visual_context], dim=0)
                ...
                textual_context = self.VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                if self.is_prior_fusion_layer and prior_prompt is not None:
                    prior_context = prior_prompt.permute(1, 0, 2).to(dtype=textual_context.dtype, device=textual_context.device)
                    textual_context = self.prior_fusion_lambda * prior_context + (1 - self.prior_fusion_lambda) * textual_context
                x = torch.cat([prefix, textual_context, suffix], dim=0)
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        self.prompt_start_layer = design_details.get("prompt_start_layer_text" if text_layer else "prompt_start_layer_vision", 0)
        ...
        elif current_trainer == 'FlexibleIVLP':
            self.resblocks = nn.Sequential(*[
                ResidualAttentionBlock_FlexibleIVLP(
                    width, heads, attn_mask,
                    (i >= self.prompt_start_layer and prompts_needed > 0),
                    text_layer, i, design_details
                )
                for i in range(layers)
            ])
    ...
    def forward(self, x):
        if self.current_trainer == 'FlexibleIVLP' and self.prompt_start_layer > 0:
...
class Transformer_VG(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.layers):
            if self.current_trainer == 'FlexibleIVLP' and self.prompt_start_layer > 0 and i < self.prompt_start_layer:
                with torch.no_grad():
                    x = self.resblocks[i](x)
            else:
                x = self.resblocks[i](x)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        self.prompt_start_layer = design_details.get("prompt_start_layer_vision", 0)
        if design_details["vision_depth"] == 0 or (
            self.current_trainer == "FlexibleIVLP" and self.prompt_start_layer > 0
        ):
    ...
    def forward(self, x: torch.Tensor, prior_prompt=None):
        ...
        if self.VPT_shallow:
            ...
            assert self.prompt_till_layer_visual == 0 or self.current_trainer == "FlexibleIVLP"
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        design_details = dict(design_details)
        trainer = design_details['trainer']
        if trainer == "FlexibleIVLP":
            vision_total_layers = vision_layers if isinstance(vision_layers, int) else 0
            design_details["prompt_start_layer_text"] = max(
                0, transformer_layers - design_details.get("language_depth", 0)
            )
            design_details["prompt_start_layer_vision"] = max(
                0, vision_total_layers - design_details.get("vision_depth", 0)
            )
        else:
            design_details["prompt_start_layer_text"] = 0
            design_details["prompt_start_layer_vision"] = 0
